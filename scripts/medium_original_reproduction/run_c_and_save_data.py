"""
Step 1: Run C (Python) MCMC methods + save shared data for R.
Output: results/medium_original_reproduction/tables/c_results.csv
"""
...
csv_path = os.path.join(RESULTS, "tables", "c_results.csv")
rows = []
...
for idx, (p, n, seed, exp_group, paper, ds) in enumerate(CASES):
    ...
    for method_name, method_fn in [("c_order", run_order_mcmc), ("c_structure", run_structure_mcmc)]:
        rng2 = RNG(seed)
        ...
        rows.append({
            ...
            "p": p, "n": n, "seed": seed,
            "method": method_name, "implementation": "C",
